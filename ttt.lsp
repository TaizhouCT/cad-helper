...
;; (RegexpReplace "$ 3.25" "\\$ (\\d+(\\.\\d+)?)" "$1 â‚¬" nil T)  ; => "3.25 â‚¬"
...
(defun x:process-replace (ss devices / attrname devname device items ssidx next ed)
  ...
  (while (/= ssidx (sslength ss))
    ...
    (if (or (not devname) (/= attrname "»ú¹ñµØÖ·"))
      (progn
...
(defun c:ttt (/ fname fp line items last-dev-items devices ss)
  ...
  ; process replace
  (setq ss (ssget "X" '((0 . "INSERT") (2 . "IOµãÎ»×¢ÊÍ"))))
  (if ss (x:process-replace ss devices))
  (setq ss (ssget "X" '((0 . "INSERT") (2 . "AIµãÎ»×¢ÊÍ"))))
  (if ss (x:process-replace ss devices))
  (setq ss (ssget "X" '((0 . "INSERT") (2 . "AOµãÎ»×¢ÊÍ"))))
  (if ss (x:process-replace ss devices))
  (setq ss (ssget "X" '((0 . "INSERT") (2 . "DIµãÎ»×¢ÊÍ"))))
  (if ss (x:process-replace ss devices))
  (setq ss (ssget "X" '((0 . "INSERT") (2 . "DOµãÎ»×¢ÊÍ"))))
  (if ss (x:process-replace ss devices))
